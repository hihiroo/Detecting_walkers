...
class World(object):
    ...
    def restart(self):
        ...
        self.camera_manager = CameraManager(self.player, self.hud, self._gamma, self.player)
        self.camera_manager.nodisplay = self.args.nodisplay
        self.camera_manager.recording = self.args.save_img
        ...
        # walker_bp = random.choice(walkers)

        # 1. Take all the random locations to spawn
        batch = []
        # walkers_cnt = self.args.walkers
        walker_cnt = 0
        # for i in range(walkers_cnt):
        #     spawn_point = carla.Transform()
        #     spawn_point.location = self.world.get_random_location_from_navigation()
        #     while spawn_point.location is None:
        #         spawn_point.location = self.world.get_random_location_from_navigation()
        #     walker_bp = random.choice(walkers)
        ...
        self.walker_all_id = []
        for i in range(len(walkers_list)):
            self.walker_all_id.append(walkers_list[i]["con"])
            self.walker_all_id.append(walkers_list[i]["id"])
        self.walkers = self.world.get_actors(self.walker_all_id)

        # wait for a tick to ensure client receives the last transform of the walkers we have just created
        self.world.wait_for_tick()

        # 5. initialize each controller and set target to walk to (list is [controller, actor, controller, actor ...])
        for i in range(0, len(self.walkers), 2):
            # start walker
            self.walkers[i].start()
            # set walk to random point
            self.walkers[i].go_to_location(self.world.get_random_location_from_navigation())
            # random max speed
            self.walkers[i].set_max_speed(1 + random.random())    # max speed between 1 and 2 (default is 1.4 m/s)

        print("Finished Walkers Spawning")
        ...
        if vehicle_cnt < number_of_spawn_points:
            random.shuffle(spawn_points)
        elif vehicle_cnt > number_of_spawn_points:
            msg = 'requested %d vehicles, but could only find %d spawn points'
            logging.warning(msg, vehicle_cnt, number_of_spawn_points)
            vehicle_cnt = number_of_spawn_points
        ...
        print("Finished Vehicle Spawning")
    ...
    def next_weather(self, reverse=False):
        ...
        weather = carla.WeatherParameters(
            cloudiness= self.args.cloud,
            precipitation = self.args.rain,
            precipitation_deposits = 0.0,
    ...
    def render(self, display):
        ...
        if not self.args.nodisplay: self.hud.render(display)
    ...
    def destroy(self):
        ...
        actors = [
            ...
            self.player,
            self.camera_manager.sensor_for_semantic]
        for actor in actors:
            if actor is not None:
                actor.destroy()
        for i in range (0, len(self.walker_all_id),2):
            self.walkers[i].stop()
        self.client.apply_batch([carla.command.DestroyActor(x) for x in self.walker_all_id])
...
class HUD(object):
    ...
    def tick(self, world, clock):
        ...
        if (not self.args.nodisplay) and self._show_info:
            return
...
class CameraManager(object):
    def __init__(self, parent_actor, hud, gamma_correction, player):
        self.sensor = None
        self.surface = None
        self._parent = parent_actor
        self.hud = hud
        ...
        for item in self.sensors:
            ...
            item.append(bp)
        self.index = None
        self.save_dir = os.getcwd()+"/image_recording"
        self.save_semantic_dir = self.save_dir + "/semantic" 
        self.sensor_for_semantic = None
    ...
    def set_sensor(self, index, notify=True, force_respawn=False):
        ...
        if needs_respawn:
            ...
            weak_self = weakref.ref(self)
            if index == 0:
                self.sensor.listen(lambda image: CameraManager._parse_image(weak_self, image, self.save_dir))
            else:
                self.sensor.listen(lambda image: CameraManager._parse_image(weak_self, image, self.save_semantic_dir))
        if notify:
            self.hud.notification(self.sensors[index][2])
        self.index = index
        
        if self.sensor_for_semantic is None:
            if not os.path.isdir(self.save_semantic_dir):
                os.makedirs(self.save_semantic_dir)
            self.sensor_for_semantic = self._parent.get_world().spawn_actor(
            self.sensors[5][-1],
            self._camera_transforms[1][0],
            attach_to=self._parent,
            attachment_type=self._camera_transforms[1][1])
            if self.sensor_for_semantic is not None:
                cc = carla.ColorConverter.CityScapesPalette
                self.sensor_for_semantic.listen(lambda image: image.save_to_disk(self.save_semantic_dir + '/%08d' % image.frame,cc))  
    ...
    def _parse_image(weak_self, image, save_dir):
        ...
        if self.recording:
            ...
            if save_dir == self.save_semantic_dir:
                cc = carla.ColorConverter.CityScapesPalette
            else:
            ...
            with open(save_dir + '/%08d' % image.frame + '.txt', 'w') as car_info:
                car_info.write("Car_velocity_xyz ")
                car_info.write(str(self.player.get_velocity().x) + " " + str(self.player.get_velocity().y) + " " + str(self.player.get_velocity().z) + '\n')
            ...
            # print(save_dir + '/%08d' % image.frame)
...
def game_loop(args):
    ...
    try:
        ...
        display = None if args.nodisplay else pygame.display.set_mode(
        ...
        client.get_world() 
        world = World(client,client.load_world(args.map), hud, args)
        ...
        while True:
            ...
            if (not args.nodisplay) and controller.parse_events(client, world, clock):
                return
            world.tick(clock)
